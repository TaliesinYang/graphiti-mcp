...
from smart_retrieval_mcp import SmartRetrievalMCP
...
# Configure logging - redirect to file to avoid interfering with MCP stdio communication
import tempfile
log_file = os.path.join(tempfile.gettempdir(), 'graphiti_mcp.log')
logging.basicConfig(
    level=logging.ERROR,  # Only log errors to reduce noise
...
# Suppress Neo4j notifications to avoid stdio interference
...
smart_retriever: SmartRetrievalMCP | None = None


async def initialize_graphiti():
    """Initialize the Graphiti client with the configured settings."""
    global graphiti_client, smart_retriever, config
    ...
    try:
        ...
        # Smart Retrieval will be initialized lazily on first use
        # to avoid slowing down MCP server startup
        logger.info('Smart Retrieval system will be initialized on first use')
...
async def smart_search_memory(
    ...
    """Smart dual search: Facts for discovery + Episodes for complete context.
    
    This tool implements intelligent search with:
    - Automatic query expansion using learned associations
    - Facts-based discovery for finding relationships
    - Episode retrieval for complete content
    - Self-learning from search results
    
    Args:
        query: The search query (supports Chinese/English mixed)
        group_ids: Optional list of group IDs to filter results
        learn: Whether to enable learning from results (default: True)
    ...
    Returns:
        ...
        - episodes: List of complete episodes
        - facts_count: Number of facts found
        - episodes_count: Number of episodes retrieved
        - search_time: Time taken for search
        - learned_terms: Terms learned from this search
    """
    global smart_retriever, graphiti_client
    
    # Lazy initialization on first use
    if smart_retriever is None:
        if graphiti_client is None:
            return ErrorResponse(error='Graphiti client not initialized')
        try:
            smart_retriever = SmartRetrievalMCP(graphiti_client)
            logger.info('Smart Retrieval system initialized on first use')
        except Exception as e:
            logger.error(f'Failed to initialize Smart Retrieval: {str(e)}')
            return ErrorResponse(error=f'Failed to initialize Smart Retrieval: {str(e)}')
    
    try:
        result = await smart_retriever.search(query, group_ids, learn)
        return result
    except Exception as e:
        error_msg = str(e)
        logger.error(f'Error in smart search: {error_msg}')
        return ErrorResponse(error=f'Error in smart search: {error_msg}')
...
async def get_episode_by_uuid(
    ...
    Args:
        uuid: The UUID of the episode to retrieve
    
    Returns:
        Episode data including content, metadata, and timestamps
    """
    global smart_retriever, graphiti_client
    ...
    try:
        episode = await smart_retriever.get_episode_by_uuid(uuid)
        if episode:
            return episode
        else:
...
@mcp.tool()
async def get_learning_stats() -> dict[str, Any] | ErrorResponse:
    """Get statistics about the smart search learning system.
    
    Returns:
        Dictionary containing:
        - total_searches: Total number of searches performed
        - learned_queries: Number of queries with learned associations
        - total_associations: Total number of term associations learned
        - recent_queries: List of recent search queries
        - top_learned: Top 5 queries with most associations
    """
    global smart_retriever, graphiti_client
    ...
    try:
        stats = smart_retriever.get_search_stats()
        return stats
    except Exception as e:
        error_msg = str(e)
        logger.error(f'Error getting learning stats: {error_msg}')
        return ErrorResponse(error=f'Error getting learning stats: {error_msg}')
